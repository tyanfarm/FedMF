...
class Engine(object):
    ...
    def aggregate_clients_params(self, round_user_params):
        ...
        item_embedding = np.zeros((len(round_user_params), self.config['num_items'] * self.config['latent_dim']), dtype='float32')
        for user in round_user_params.keys():
            item_embedding[user] = round_user_params[user]['embedding_item.weight'].numpy().flatten()
        self.server_model_param['embedding_item.weight']['global'] = torch.from_numpy(
            np.mean(item_embedding, axis=0).reshape(self.config['num_items'], self.config['latent_dim']))
        del item_embedding
